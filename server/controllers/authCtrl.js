...
const authCtrl = {
  ...
  createClinics: async (req, res) => {
    try {
      ...
      if (!name || !address || !medicalWorker) {
        return res
          .status(400)
          .json({ message: 'Bad request: name, medicalWorker, and address are required' });
      }
  ...
  registerMedicalWorker: async (req, res) => {
    try {
      ...
      if (role !== 'medicalStoreWorker')
        return res.status(400).json({ msg: 'Unauthorized role selection.' });
