...
app.post('/api/clinics', auth, upload.single('image'), (req, res) => {
...
app.get('/api/my-clinics', auth, async (req, res) => {
...
app.get('/api/bookings', auth, async (req, res) => {
  try {
    const userId = req.user.id;
    const allClinics = await Clinic.findOne({ createdBy: userId });
    if (!allClinics) {
      return res.status(404).json({ msg: 'Clinic not found' });
    }
    const bookings = (await Booking.find({ clinicId: allClinics._id })) || [];
...
// Fallback route
app.get('*', (req, res) => {
  res.sendFile(path.resolve(__dirname, '../ds/client/build/index.html'), function (err) {
    if (err) {
