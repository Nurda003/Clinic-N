            ...
            <div className="w-11/12 mx-auto">
                ...
                <div className="flex flex-col items-center py-5">
                    ...
                    {bookings.map((booking) => (
                        <div key={booking._id} className="flex w-full gap-4 p-4 rounded-2xl bg-white items-center mb-4">
                            <div className="w-48 h-48 flex-shrink-0">
                                {booking.clinicId?.image?.filename
                                    ? <img src={`/api/image/${booking.clinicId.image.filename}`} alt={booking.clinicId.name} className="w-full h-full object-cover rounded-lg" />
                                    : <img src={cli1} alt="Default Clinic" className="w-full h-full object-cover rounded-lg" />
                                }
                            </div>
                            <div className="flex-1 p-3">
                                <div className="flex justify-between">
                                    ...
                                    <div className="flex flex-col gap-1 text-right">
                                        ...
                                        <p className='text-xl text-blue-700 font-semibold'>{booking.firstName} {booking.lastName}</p>
                                        <p className='text-base text-smalltext'>On: {new Date(booking.date).toLocaleDateString()}</p>
                                        {booking.message && <p className='text-base text-smalltext'>Message: {booking.message}</p>}
                                    </div>
                                </div>
